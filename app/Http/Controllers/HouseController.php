...
class HouseController extends Controller
{
    public function show_lend(){
       ...
       $posts=DB::table('posts')->orderBy('id','desc')->get();
        return view('lend.index',['posts'=>$posts,'houses'=>$houses,'feedbacks'=>$feedbacks]);
    }
    ...
    public function store_reply_comments(Request $request,$house){
        if(Auth::check()){
            $house=House::find($house);
            $houseid=$house->id;
    ...
    public function show_house($houseId)
    {
        ...
        $replycomments=ReplyComment::where('house_id',$houseId)->get();
        $categories = Category::all();
        $other_houses = House::where('id', '!=', $houseId)->orderBy('created_at','desc')->with('imagePosts','comforts')->get();
